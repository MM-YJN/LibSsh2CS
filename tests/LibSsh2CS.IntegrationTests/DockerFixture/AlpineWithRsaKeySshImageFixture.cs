...
// Marks AlpineWithRsaKeySshImageFixture as an assembly-wide fixture: xUnit
...
[assembly: AssemblyFixture(typeof(AlpineWithRsaKeySshImageFixture))]

namespace LibSsh2CS.IntegrationTests.DockerFixture;

/// <summary>
/// Assembly-wide fixture that builds the Alpine 3.20 image with the test
/// RSA public key (<c>Fixtures/rsa_plain_key.pub</c>) installed in
/// <c>authorized_keys</c> on first use and retains it across
/// test runs. Used by the RSA publickey auth
/// test (which exercises the RSA-SHA2 algorithm selection path via
/// EXT_INFO <c>server-sig-algs</c>). Test classes receive it via
/// constructor injection and start a fresh per-test container via
/// <see cref="SshImageFixtureBase.StartContainerAsync"/>.
/// </summary>
public sealed class AlpineWithRsaKeySshImageFixture(IMessageSink messageSink)
    : SshImageFixtureBase(messageSink)
{
    ...
    /// <summary>
    /// The test RSA public key, loaded from the embedded fixture resource
    /// at build time and baked into the image's <c>authorized_keys</c>.
    /// </summary>
    protected override string? AuthorizedKey => FixtureLoader.LoadText("rsa_plain_key.pub");
}
